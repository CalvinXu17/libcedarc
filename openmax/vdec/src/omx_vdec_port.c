...
OMX_BUFFERHEADERTYPE* doRequestPortBuffer(AwOmxVdecPort* mPort)
{
    ...
    if(pBufHdr == NULL)
    {
        omx_loge("fatal error! (%s)pBufHdr is NULL, check code!",
          isInputPort(mPort)?"<<<<<<<<in":">>>>>>>out");
        OmxReleaseMutex(mPort->m_bufMutex);
        return NULL;
    }
    mPort->m_sBufList.nSizeOfList--;
    OMX_ASSERT(mPort->m_sBufList.nSizeOfList <= mPort->m_sBufList.nAllocSize);
                                          ...
                                          OMX_PARAM_PORTDEFINITIONTYPE *value)
{
    ...
    if(mPort->m_sPortDefType.format.video.nFrameWidth == 322 && mPort->m_sPortDefType.format.video.nFrameHeight == 2)
    {
        omx_loge("we would report OMX_ErrorHardware for frame width [%lu] and height [%lu]",
                                          ...
                                          OMX_PARAM_PORTDEFINITIONTYPE *value)
{
    ...
    if(value->nBufferCountActual != mPort->m_BufferCntActual)
    {
        ...
        omx_logd("****port, extbuf:%ld", mPort->mExtraBufferNum);
        OmxAcquireMutex(mPort->m_bufMutex);
                                  ...
                                  OMX_VIDEO_PARAM_PROFILELEVELTYPE *value)
{
    ...
    case OMX_VIDEO_CodingH263:
        pProfileLevel = DecoderSupportedH263ProfileLevels;
        nNumberOfProfiles
            = sizeof(DecoderSupportedH263ProfileLevels) / sizeof (VIDEO_PROFILE_LEVEL_TYPE);
        break;

    case OMX_VIDEO_CodingMPEG4:
        pProfileLevel = DecoderSupportedMPEG4ProfileLevels;
        nNumberOfProfiles
            = sizeof(DecoderSupportedMPEG4ProfileLevels) / sizeof (VIDEO_PROFILE_LEVEL_TYPE);
        break;

    case OMX_VIDEO_CodingAVC:
        pProfileLevel = DecoderSupportedAVCProfileLevels;
        nNumberOfProfiles
            = sizeof(DecoderSupportedAVCProfileLevels) / sizeof (VIDEO_PROFILE_LEVEL_TYPE);
        break;

    case OMX_VIDEO_CodingHEVC:
        pProfileLevel = DecoderSupportedHEVCProfileLevels;
        nNumberOfProfiles
            = sizeof(DecoderSupportedHEVCProfileLevels) / sizeof (VIDEO_PROFILE_LEVEL_TYPE);
        break;
    case OMX_VIDEO_CodingMPEG2:
        pProfileLevel = DecoderSupportedMPEG2ProfileLevels;
        nNumberOfProfiles
            = sizeof(DecoderSupportedMPEG2ProfileLevels) / sizeof (VIDEO_PROFILE_LEVEL_TYPE);
        break;
    default:
...
{
    OMX_S8 nIndex = 0x0;
    omx_logd("%s port add buffer: %p", isInputPort(mPort)?"<<<<<<<<in":">>>>>>>out", pBuffer);
