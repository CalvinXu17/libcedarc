...
#if ENABLE_SHOW_BUFINFO_STATUS
    omx_logd("%s. line %d.", __func__, __LINE__);
    showBufferInfoArrStatus(pCtx);
#endif
...
static void freeOutputBufferInfoArr(OmxAwDecoderContext *pCtx)
{
    ...
    for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
    {
        ...
        {
            //omx_logd("ion_free: handle_ion[%p],i[%d]",handle_ion,i);
                                 ...
                                 OMX_BUFFERHEADERTYPE* pInBufHdr)
{
    ...
    OmxAcquireMutex(pCtx->awMutexHandler);
    maxVideoStreamBufferSize = VideoStreamBufferSize(pCtx->m_decoder, 0);
    if(require_size > maxVideoStreamBufferSize || require_size <= 0)
    {
        omx_loge("unexpected required buffer size with [%d]! maxVideoStreamBufferSize [%d].",
    ...
    {
        omx_loge("req vbs fail! maybe vbs buffer is full! require_size[%d]",
             require_size);
    ...
    {
        omx_logw(" the requestSize[%d] is not equal to needSize[%d]!",
...
static inline void __anFlush(OmxDecoder* pDec)
{
    ...
    {
        omx_logw(" fatal error, m_decoder is not malloc when flush all ports!");
    }
    if(pCtx->bUseZeroCopyBuffer)
    {
        pCtx->pCurPicture    = NULL;
...
OmxDecoder* OmxDecoderCreate(AwOmxVdecPort* in, AwOmxVdecPort* out, OMX_BOOL bIsSecureVideoFlag)
{
    ...
    else
    {
        ...
        omx_logd("** nPhyOffset = %lx",pCtx->nPhyOffset);
    }
