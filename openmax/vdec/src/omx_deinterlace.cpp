...
#if defined(CONF_DI_300_SUPPORT)
static int modeSetting(int fd, struct di_size *size,
                        struct di_rect *rect,
...
static int preProcess(struct DiContext* dc)
{
                        ...
                        unsigned int fmtOut)
{
    ...
    mDiParaT3->top_field_first = pCurPicture->bTopFieldFirst;
    ...
    {
        fb->dma_buf_fd = -1;
        fb->buf.y_addr = pPrePrePicture->phyYBufAddr + dc->nPhyOffset;
        fb->buf.cb_addr = pPrePrePicture->phyCBufAddr + dc->nPhyOffset;
    ...
    {
        fb->dma_buf_fd = pPrePrePicture->nBufFd;
        fb->buf.y_addr = 0;
        ...
        || (dc->tnr_en == 1))
    {
        ...
        if (dc->iomemType == IOMEM_CMA)
        {
            ...
            fb->buf.y_addr = pPrePicture->phyYBufAddr + dc->nPhyOffset;
            fb->buf.cb_addr = pPrePicture->phyCBufAddr + dc->nPhyOffset;
            if (fmtOut == DI2_FORMAT_YV12)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 4;
            else if(fmtOut == DI2_FORMAT_YUV422P)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 2;
        ...
        {
            fb->dma_buf_fd = pPrePicture->nBufFd;
            fb->buf.y_addr = 0;
            fb->buf.cb_addr = fb->size.width * fb->size.height;
    ...
    if (dc->mode == DI_MODE_60HZ)
    {
        ...
        if (dc->iomemType == IOMEM_CMA)
        {
            ...
            fb->buf.y_addr = pCurPicture->phyYBufAddr + dc->nPhyOffset;
            fb->buf.cb_addr = pCurPicture->phyCBufAddr + dc->nPhyOffset;
            if (fmtOut == DI2_FORMAT_YV12)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 4;
            else if(fmtOut == DI2_FORMAT_YUV422P)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 2;
        ...
        {
            fb->dma_buf_fd = pCurPicture->nBufFd;
            fb->buf.y_addr = 0;
            fb->buf.cb_addr = fb->size.width * fb->size.height;
    ...
    if (dc->mode != DI_MODE_TNR)
    {
        ...
        {
            fb->dma_buf_fd = -1;
            fb->buf.y_addr = pOutPicture0->phyYBufAddr + dc->nPhyOffset;
            fb->buf.cb_addr = pOutPicture0->phyCBufAddr + dc->nPhyOffset;
            if (fmtOut == DI2_FORMAT_YV12)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 4;
            else if(fmtOut == DI2_FORMAT_YUV422P)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 2;
        ...
        {
            fb->dma_buf_fd = pOutPicture0->nBufFd;
            fb->buf.y_addr = 0;
            fb->buf.cb_addr = fb->size.width * fb->size.height;
    ...
    if (dc->mode == DI_MODE_60HZ)
    {
        ...
        {
            fb->dma_buf_fd = -1;
            fb->buf.y_addr = pOutPicture1->phyYBufAddr + dc->nPhyOffset;
            fb->buf.cb_addr = pOutPicture1->phyCBufAddr + dc->nPhyOffset;
            if (fmtOut == DI2_FORMAT_YV12)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 4;
            else if(fmtOut == DI2_FORMAT_YUV422P)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 2;
        ...
        {
            fb->dma_buf_fd = pOutPicture1->nBufFd;
            fb->buf.y_addr = 0;
            fb->buf.cb_addr = fb->size.width * fb->size.height;
    ...
    if ((dc->mode == DI_MODE_TNR) || dc->tnr_en)
    {
        ...
        {
            fb->dma_buf_fd = -1;
            fb->buf.y_addr = pOutTnr->phyYBufAddr + dc->nPhyOffset;
            fb->buf.cb_addr = pOutTnr->phyCBufAddr + dc->nPhyOffset;
            if (fmtOut == DI2_FORMAT_YV12)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 4;
            else if(fmtOut == DI2_FORMAT_YUV422P)
                fb->buf.cr_addr = fb->buf.cb_addr + fb->size.width * fb->size.height / 2;
                    ...
                    int nField)
{
    ...
    int mAlignSizeWidth  = pOutPicture0->nLineStride;
    int mAlignSizeHeight = pOutPicture0->nHeight;
...
*/
    ...
    //* check parameters
    if(dc->tnr_en == 1 && (eOutFormat != DI2_FORMAT_YV12 &&  eOutFormat != DI2_FORMAT_YUV422P))
    {
