...
static OMX_BOOL EatThreadEntry(void *priv){
    if (priv == NULL)
        ...
        logd("*****thread eating: times:%d, id=%d, name=%s, age=%d ------",
             ++p->times, p->id, p->name, p->age);
        usleep(3000);
    }
    return OMX_TRUE;
}
...
static OMX_BOOL WorkThreadEntry(void *priv){
    ...
    logd("thread working: times:%d, id=%d, name=%s, age=%d +++++++",
          ++p->times, p->id, p->name, p->age);
    usleep(3000);
    return OMX_TRUE;
}
...
static OMX_BOOL SleepThreadEntry(void *priv){
    ...
    logd("thread sleeping: times:%d, id=%d, name=%s, age=%d  ******",
          ++p->times, p->id, p->name, p->age);
    usleep(5000);
    return OMX_TRUE;
}
