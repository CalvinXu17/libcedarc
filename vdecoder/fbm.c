...
nWidth=%d, nHeight=%d nAlignStride = %d",
    ...
    for(i=0; i<p->nMaxFrameNum; i++)
    {
            ...
            pFbmCreateInfo->nBufferType==BUF_TYPE_ONLY_REFERENCE)
        {
            ...
            {
                logv("** call allocate pic buf, i = %d, maxNum = %d",i,p->nMaxFrameNum);
        ...
        if(pFbmCreateInfo->bGpuBufValid==1)
        {
            ...
            logv("*** calcute nLower2BitBufOffset = %d(%0.2f), stride = %d",
        ...
        case PIXEL_FORMAT_NV12:
        {
            ...
            if(pPicture->b10BitPicFlag)
            {
                if(pPicture->bEnableAfbcFlag == 1)
                {
                    ...
                    logv("TotalSize: %d(%0.2f MB),AfbcSize: %d(%0.2f MB),Low2Size: %d(%0.2f MB)",
            ...
            logv("pPicture->bEnableAfbcFlag = %d",pPicture->bEnableAfbcFlag);
            if(pPicture->bEnableAfbcFlag == 1)
            {
