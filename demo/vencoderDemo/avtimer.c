...
static int64_t getNowUs_time(void)
{
    struct timeval tv;
...
static int AvTimerStart(AvTimer* t)
{
    ...
    pAvTimerCtx->nStartTimeOs = getNowUs_time();
...
{
    int64_t nCurrentTime = getNowUs_time();
...
static int64_t AvTimerGetTime(AvTimer* t)
{
    ...
    if(pAvTimerCtx->eStatus == TIMER_STATUS_START)
    {
        ...
        logv("**nStartTimeMedia = %lld, nPassedTime = %lld, \
            nCurrentTime = %lld, nStartTimeOs = %lld ",
             pAvTimerCtx->nStartTimeMedia,
             nPassedTime,
             nCurrentTime,
             pAvTimerCtx->nStartTimeOs);
        nPassedTime += pAvTimerCtx->nStartTimeMedia;
        pAvTimerCtx->nStartTimeMedia = nPassedTime;
        pAvTimerCtx->nStartTimeOs = nCurrentTime;
    }
